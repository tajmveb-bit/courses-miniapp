...
import type { ForecastEvent } from "@/data/matrixForecastEvents";

interface ForecastEventSheetProps {
  event: ForecastEvent | null;
  onClose: () => void;
}

export default function ForecastEventSheet({ event, onClose }: ForecastEventSheetProps) {
  return (
    <AnimatePresence>
      {event && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          >
            ...
            <div className="mt-6 pb-2">
              ...
              <p className="mt-5 text-sm font-semibold text-ink">В плюсовом проявлении</p>
              <p className="mt-2 text-sm leading-relaxed text-ink-soft">{event.plus}</p>
              <p className="mt-5 text-sm font-semibold text-ink">В минусовом проявлении</p>
              <p className="mt-2 text-sm leading-relaxed text-ink-soft">{event.minus}</p>
            </div>
          </motion.div>
        </div>
      )}
